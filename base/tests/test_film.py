...
class FilmViewTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(username='hostuser', email='hostuser@example.com', password='testpassword')
    ...
    def test_film_view(self):
        ...
        self.assertContains(response, '<span class="film__categories">Test Category</span>')        
        self.assertContains(response, f'<span class="thread__date">{expected_date} ago</span>')
    ...
    def test_film_details(self):
        ...
        expected_release_date = timezone.now().strftime('%Y-%m-%d')#'2023-12-12' You can make it Naive DateTime. A string
