...
class TestFilmJunkiezUser(BaseFilmJunkiezTestCase):
    def test_userProfile_rendering(self):
        ...
        assert expected_avatar_tag in markup, f"Expected: {expected_avatar_tag}\nActual: {markup}" 
...
class TestFilmJunkiezModifyUser(BaseFilmJunkiezTestCase):
    ...
    def test_modify_user_form_submission_valid_data(self):
        # Create a new user
        user = User.objects.create(username='new_username', email='new_email@example.com')
    ...
    def test_modify_user_file_upload(self):
        ...
        data = {'field1': 'value1', 'field2': 'value2', 'profile_pic': SimpleUploadedFile('test.jpg', b'file_content')}
